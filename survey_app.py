...
# 설문 문항 및 가중치 리스트
survey_questions = [
    ("나는 변수를 선언하고, 데이터 타입(문자열, 리스트, 딕셔너리 등)의 차이를 이해하여 적절히 사용할 수 있다.", 2),
    ("나는 if-else 조건문과 for/while 반복문을 활용하여 주어진 문제를 해결하는 코드를 작성할 수 있다.", 2),
    ("나는 업무에서 반복적으로 수행하는 작업(엑셀 데이터 정리, 보고서 자동화 등)을 파이썬으로 자동화할 수 있다.", 3),
    ("나는 필요한 파이썬 라이브러리(Numpy, Pandas 등)를 설치하고, 환경 설정을 올바르게 구성할 수 있다.", 3),
    ("나는 패키지 설치 중 발생하는 오류를 분석하고 해결할 수 있으며, 가상환경을 활용할 수 있다.", 3),
    ...
    ("나는 결측치 처리(삭제, 대체)와 정규화/표준화 등의 데이터 전처리 기법을 이해하고 있으며, 데이터 품질을 개선할 수 있다.", 6),
    ("나는 지도 학습(Supervised Learning)과 비지도 학습(Unsupervised Learning)의 차이를 이해하고 있으며, 적절한 상황에서 활용할 수 있다.", 6),
    ("나는 주요 머신러닝 알고리즘(로지스틱 회귀, 랜덤포레스트, SVM 등)의 기본 개념을 이해하고 있으며, 이를 실제 데이터에 적용할 수 있다.", 6),
    ("나는 파이썬의 Scikit-Learn을 사용하여 모델을 학습시키고, 평가 지표(정확도, RMSE 등)를 분석할 수 있다.", 6),
    ("나는 과적합(Overfitting)과 과소적합(Underfitting)의 개념을 이해하고, 하이퍼파라미터 조정을 통해 모델을 개선할 수 있다.", 6),
    ...
    ("나는 컴퓨터비(Computer Vision) 관련 기술(OpenCV, TensorFlow/Keras 등)을 사용하여 이미지 데이터를 분석한 경험이 있다.", 7),
